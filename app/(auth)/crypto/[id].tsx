...
const Page = () => {
  ...
  const animatedText = useAnimatedProps(() => {
    return {
      text: `${(90 * state.y.price.value.value).toFixed(0)} ₹`,
      defaultValue: '',
    };
  });
      ...
      <SectionList
        ...
        renderItem={({ item }) => (
          <>
                  ...
                  {!isActive && (
                    <View>
                      <Text style={{ fontSize: 30, fontWeight: 'bold', color: Colors.dark }}>
                        {tickers[tickers.length - 1].price.toFixed(0) * 90} ₹
                      </Text>
            ...
            <View style={[defaultStyles.block, { marginVertical: 20 }]}>
              <Text style={styles.subtitle}>Overview</Text>
              <Text style={{ color: Colors.gray }}>
                Bitcoin is a decentralized digital currency, 
                without a central bank or single administrator,
                that can be sent from user to user on the peer-to-peer bitcoin
                network without the need for intermediaries. 
                Transactions are verified by network
                nodes through cryptography and recorded in a 
