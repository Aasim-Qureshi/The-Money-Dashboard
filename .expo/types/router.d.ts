...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(auth)/` | `/(auth)/(tabs)` | `/(tabs)` | `/_sitemap` | `/crypto` | `/help` | `/home` | `/invest` | `/lifestyle` | `/login` | `/signup` | `/transfers`;
  type DynamicRoutes<T extends string> = `/(auth)/crypto/${SingleRoutePart<T>}` | `/crypto/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/(auth)/crypto/[id]` | `/crypto/[id]`;
